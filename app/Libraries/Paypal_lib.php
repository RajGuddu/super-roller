...
class paypal_lib {
	...
	public $paypal = ''; 		// load the paypal config
	public $paypal_url;
	...
	function __construct()
	{
		...
		$sanbox = $paypal->sandbox;
		$this->paypal_url = ($sanbox == TRUE)?'https://www.sandbox.paypal.com/cgi-bin/webscr':'https://www.paypal.com/cgi-bin/webscr';
                
		$this->last_error = '';
		$this->ipn_response = '';

		$this->ipn_log_file = $paypal->paypal_lib_ipn_log_file;
		$this->ipn_log = $paypal->paypal_lib_ipn_log; 
		
		$this->button_path = $paypal->paypal_lib_button_path;
		
		// populate $fields array with a few default values.  See the paypal
		// documentation for a list of fields and their data types. These defaul
		// values can be overwritten by the calling script.
		$businessEmail = $paypal->business;
		$this->add_field('business',$businessEmail);
		$this->add_field('rm','2');			  // Return method = POST
		$this->add_field('cmd','_xclick');
		
		$this->add_field('currency_code', $paypal->paypal_lib_currency_code);
